...
class SegmentationEngine:
    ...
    def get_mask(self, frame_bgr):
        ...
        # Lower threshold to capture any loose clothing/hair when moving slightly
        _, hard = cv2.threshold(mask, 0.20, 1.0, cv2.THRESH_BINARY)
        hard8 = (hard * 255).astype(np.uint8)
        
        # 19x19 kernel ensures complete silhouette coverage so when you move slightly, your body NEVER shows!
        ...
        mask = np.clip(mask * 1.5, 0, 1).astype(np.float32)
...
class PortalBox:
    ...
    def render(self, frame, seg_mask, bg, all_points=None):
        ...
        if alpha > 0.01 and bg is not None:
            ...
            roi_bg   = bg
            eff_mask = seg_mask if alpha < 0.97 else np.minimum(seg_mask * 1.8, 1.0)
            ...
            # Scanlines removed per user request: "dont do any animation only vanish myself"
